...
Deno.test("return an empty object when nothing to merge", () => {
  assertEquals(deepmerge(), {});
});

Deno.test("return the same object if only 1 is passed", () => {
  const foo = { prop: 1 };
  assertStrictEquals(deepmerge(foo), foo);
});

Deno.test("can merge objects with different props", () => {
  const v = { first: true };
...
Deno.test(`can merge nested objects`, () => {
  ...
  assertEquals(deepmerge(x, y), expected);
});
